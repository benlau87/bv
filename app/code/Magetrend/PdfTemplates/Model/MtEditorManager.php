...
class MtEditorManager
{
    ...
    public function createTemplate($params)
    {
        ...
        if (isset($params['template_design']) && !empty($params['template_design'])) {
            ...
            $fileName = $design.'.xml';
            $filePath = 'Magetrend_PdfTemplates::design/'.$templateType.'/'.$size.'/'.$fileName;
    ...
    {
        $xml = $this->getDesignXml($filePath, $fileName);
        if (!$xml) {
            return false;
        }
     ...
     * @return bool
     */
    public function getDesignXml($filePath, $fileName)
    {
        $fileContent = $this->readFactory->create($filePath)
            ->readFile($fileName);
        $mediaUrl = $this->assetRepo->getUrl('Magetrend_PdfTemplates::images/media/logo.jpg');
        $mediaUrl = str_replace('logo.jpg', '', $mediaUrl);
        $fileContent = str_replace('Magetrend_PdfTemplates::images/media/', $mediaUrl, $fileContent);
    ...
    public function prepareForImport($value)
    {
        if (substr_count($value, 'images/media/') == 1) {
            ...
            $value = $this->assetRepo->createAsset('Magetrend_PdfTemplates::images/media/'.$value[1])
                ->getUrl();
    ...
    public function getAvailableVariableList($templateId, $sourceId)
    {
        ...
        switch ($template->getType()) {
            case 'order':
                $sourceObject = $this->orderRepository->get($sourceId);
                break;
            case 'invoice':
                $sourceObject = $this->invoiceRepository->get($sourceId);
                break;
            case 'shipment':
                $sourceObject = $this->shipmentRepository->get($sourceId);
                break;
            case 'creditmemo':
                $sourceObject = $this->creditmemoRepository->get($sourceId);
                break;
            default:
                throw new \Exception(__('Unknow source type'));
        }
        $filter =  $this->getFilter($sourceObject)->setSource($sourceObject);
    ...
    public function getFilter($source)
    {
