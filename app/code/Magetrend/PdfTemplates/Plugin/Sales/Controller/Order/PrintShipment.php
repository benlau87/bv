    ...
    public function aroundExecute($subject, $parent)
    {
        ...
        if ($this->orderAuthorization->canView($order)) {
            if (isset($shipment)) {
                ...
                $fileName = sprintf('shipment_%s.pdf', $this->moduleHelper->prepareFileName($shipment->getIncrementId()));
            } else {
                $shipmentCollection = $order->getShipmentsCollection();
                if ($shipmentCollection->getSize() > 0) {
                    $fileName = sprintf('order_%s_shipments.pdf', $this->moduleHelper->prepareFileName($order->getIncrementId()));
