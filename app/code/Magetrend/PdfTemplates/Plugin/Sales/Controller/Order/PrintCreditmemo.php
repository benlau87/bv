    ...
    public function aroundExecute($subject, $parent)
    {
        ...
        if ($this->orderAuthorization->canView($order)) {
            if (isset($creditMemo)) {
                ...
                $fileName = sprintf('credit_memo_%s.pdf', $this->moduleHelper->prepareFileName($creditMemo->getIncrementId()));
            } else {
                $creditMemoCollection = $order->getCreditmemosCollection();
                if ($creditMemoCollection->getSize() > 0) {
                    $fileName = sprintf('order_%s_credit_memos.pdf', $this->moduleHelper->prepareFileName($order->getIncrementId()));
