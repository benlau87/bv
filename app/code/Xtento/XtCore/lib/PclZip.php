 ...
 * File:          app/code/Xtento/XtCore/lib/PclZip.php
 * Copyright:     Copyright (c) 2018 XTENTO GmbH & Co. KG <info@xtento.com> / All rights reserved.
 */

// @codingStandardsIgnoreFile
namespace Xtento\XtCore\lib;
...
class PclZip
{
    ...
    function PclZip($p_zipname)
    {
        ...
        if (!function_exists('gzopen')) {
            return false;//('Abort ' . basename(__FILE__) . ' : Missing zlib extensions');
        }
...
{
    $pos = strpos($p_path, ':');
    if (($p_remove_disk_letter) && (($v_position = $pos) != false)) {
        $p_path = substr($p_path, $v_position + 1);
    }
    $pos = strpos($p_path, '\\');
    if (($pos > 0) || (substr($p_path, 0, 1) == '\\')) {
        $p_path = strtr($p_path, '\\', '/');
    }
    return $p_path;
}